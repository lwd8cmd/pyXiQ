...
static PyObject* set_table (PyObject *dummy, PyObject *args) {
	PyObject *arg1=NULL;
...
static PyObject* segment (PyObject *dummy, PyObject *args) {
	...
	for (i=0; i<yuv->dimensions[0]; ++i) {
		for (j=0; j<yuv->dimensions[1]; ++j) {
			...
			//unsigned char *r = (unsigned char*)PyArray_GETPTR3(lookup_table, *y, *u, *v);
			//unsigned char *r = (unsigned char*)PyArray_GETPTR1(lookup_table, *y + *u * 256 + *v * 65536);
			//unsigned char *r = (unsigned char*)PyArray_GETPTR1(lookup_table, *y + (*u << 8) + (*v << 16));
...
PyMODINIT_FUNC
initsegment (void) {
	(void)Py_InitModule("segment", methods);
	import_array();
}